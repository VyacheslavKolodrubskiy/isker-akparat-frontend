...
export default {
  ...
  computed: {
    ...
    showAssignManager() {
      return this.$canUse(['admin', 'manager']) && !this.application.manager &&
  ...
  methods: {
    ...
    readApplication() {
      getAxios(`/application/${this.$route.params.id}`, {}, { globalLoading: true }).then((res) => {
    ...
    closeApplication() {
      this.$confirm({
        ...
        cancelText: 'Нет',
        onOk: () => {
          getAxios(`/application/close/${this.application.id}`, {}, { globalLoading: true })
            .then((res) => {
              this.readApplication();
    ...
    submitManagerForm(values) {
      postAxios(`/application/assign/${this.application.id}`, values, { globalLoading: true })
        .then((res) => {
          this.$notification.success({
            message: 'Назначен',
          });
    ...
    submitReply(values) {
      postAxios(`/application/reply/${this.application.id}`, values, { formDataConvert: true, globalLoading: true })
        .then(() => {
          this.$notification.success({
            message: 'Сохранено',
          });
