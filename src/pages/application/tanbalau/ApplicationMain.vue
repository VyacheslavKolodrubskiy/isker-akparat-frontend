<template>
  <div>
    <APageHeader title="Заявки">
      <template #extra>
        <AddLink v-if="$canUse(['client'])" :to="{ name: 'ApplicationTanbalauAdd' }" />
      </template>
    </APageHeader>
...
export default {
  ...
  data() {
    return {
      ...
      actions: [
        {
          title: 'Просмотр',
          to: (record) => ({ name: 'ApplicationTanbalauView', params: { id: record.id } }),
        },
      ],
    };
  },
  ...
  methods: {
    ...
    fetchApplications(params = {}) {
      return getAxios('/application/tanbalau', { ...params }).then((res) => {
