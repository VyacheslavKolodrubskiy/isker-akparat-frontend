...
import { fastClone } from '@/utils/utils';
import { APPLICATION_STATUSES, PRODUCT_STATUSES, ORDER_STATUSES } from '@/config/types';
  ...
  computed: {
    applicationChart() {
      return {
        ...
        backgroundColor: APPLICATION_STATUSES.values.map(el => el.hexColor),
    ...
    npcChart() {
      return {
        ...
        backgroundColor: PRODUCT_STATUSES.values.map(el => el.hexColor),
        data: this.npc,
      };
    },
    orderChart() {
      return {
        ...
        backgroundColor: ORDER_STATUSES.values.map(el => el.hexColor),
        data: this.suz,
      };
    },
  },
  ...
  methods: {
    setChart(chart) {
      ...
      chartOptions.plugins.datalabels = {
        display: (context) => {
          let total = context.dataset.data.reduce((prev, curr) => prev + curr);
    ...
    fetchPrintedKmCount(params = {}) {
      return getAxios(
        ...
        this.appLables = APPLICATION_STATUSES.values.map((el, i) => el.title + ': ' + this.applications[i]);
        ...
        this.npcLables = PRODUCT_STATUSES.values.map((el, i) => el.title + ': ' + this.npc[i]);
        ...
        this.suzLables = ORDER_STATUSES.values.map((el, i) => el.title + ': ' + this.suz[i]);
